...
$this->title = Yii::t('app', 'Submitted Individual Assignments');
...
foreach ($models as $model): ?>
    ...
    ['/submission/mark-individual', 'AssignmentID' => $model->AssignmentID],
    ...
    <?php else: ?>
            <?php foreach ($dataProvider->models as $model): ?>
                    <?=Html::a(
    '<blockquote class="row justify-content-around rounded-5 p-1 m-1 elevation-4">
        <div class="col-sm-6 text-center">
            <h3 class="m-1">' . Html::encode($model->student->fname . ' ' . $model->student->mname . ' ' . $model->student->lname) . '</h3>
            <p>
                <footer class="blockquote-footer">REG NO:
                    <cite title="Source Title">' . Html::encode($model->student->regNo) . '</cite>
                </footer>
            </p>
        </div>
